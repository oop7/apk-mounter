...
class RootAPI {
  final String _mountDirPath = '/data/adb/apk-mounter';
  ...
  Future<bool> isRooted() async {
    try {
      ...
      // Fallback for KernelSU/APatch where binary is hidden
  ...
  Future<List<String>> getInstalledApps() async {
    ...
    try {
      final String? res = await Root.exec(cmd: 'ls $_mountDirPath');
      if (res != null) {
  ...
  Future<void> uninstall(String packageName) async {
    await Root.exec(
      cmd: '''
          ...
          rm -rf $_mountDirPath/$packageName $_serviceDDirPath/$packageName.sh
          ''',
    );
  }
  ...
  Future<void> removeOrphanedFiles() async {
    await Root.exec(
      cmd: 'find $_mountDirPath -type f -name original.apk -delete',
    );
  }
  ...
  Future<void> installServiceDScript(String packageName) async {
    ...
    base_path=$_mountDirPath/$packageName/base.apk
  ...
  Future<void> installPatchedApk(
    ...
    final String newPatchedFilePath = '$_mountDirPath/$packageName/base.apk';
    await Root.exec(
      cmd: '''
      mkdir -p $_mountDirPath/$packageName
